...
#define MKFUN_UNPACK_SAVE_DATA_TYPE1(TYPE, NAME, MED)               \
    ...
    {                                                               \
        ...
        *ds_id = (char*)MEM_Malloc(strlen(id)+1);                       \
...
#define UNPACK_SAVE_DATA_TYPE1_END_STRING                               \
    ...
    *value = (char*)MEM_Malloc(strlen(valuestring)+1);                      \
...
static void FormatAt(char* buffer, int len, time_t now)
{
    ...
    OS_CRITICAL_ENTER();
    timinfo = localtime((const time_t*)&now);
    strftime(buffer, len, "%F %T", timinfo); 
    OS_CRITICAL_EXIT();
#endif
...
Buffer* NewBuffer()
{
    Buffer* buf = (Buffer*)MEM_Malloc(sizeof(Buffer));
    buf->_data = (uint8*)MEM_Malloc(sizeof(uint8) * BUFFER_SIZE);
    buf->_write_pos = 0;
    buf->_read_pos = 0;
...
int32 CheckCapacity(Buffer* buf, uint32 len)
{
    ...
    {
        pdata = (uint8*)MEM_Malloc(sizeof(uint8) * cap_len);
...
int32 ReadBytes(EdpPacket* pkg, uint8** val, uint32 count)
{
    if (pkg->_read_pos+count > pkg->_write_pos) 
        return -1;
    *val = (uint8*)MEM_Malloc(sizeof(uint8) * count);
...
int32 ReadStr(EdpPacket* pkg, char** val)
{
    ...
    if (pkg->_read_pos+len > pkg->_write_pos) 
        return -1;
    /* copy str val */
    *val = (char*)MEM_Malloc(sizeof(char) * (len + 1));
    memset(*val, 0, len+1);
...
int32 UnpackSavedataFloatWithTime(EdpPacket* pkg, FloatDPS** output, int* out_count, LPDataTime at){
    ...
    *out_count = count;
    *output = (FloatDPS*)MEM_Malloc(*out_count * sizeof(FloatDPS));
    if (NULL == *output){
        return ERR_UNPACK_SAVED_FLOAT_WITH_TIME;
    }
...
int UnpackUpdateResp(EdpPacket* pkg, UpdateInfoList** head)
{
    ...
    while (pkg->_read_pos != pkg->_write_pos){
        if (node){
            node->next = (UpdateInfoList*)MEM_Malloc(sizeof(UpdateInfoList));
            node = node->next;
        }
        else{
            *head = (UpdateInfoList*)MEM_Malloc(sizeof(UpdateInfoList));
            node = *head;
        }
        node->next = NULL;
