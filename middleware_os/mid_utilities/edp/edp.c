...
edp_pkt *packetCreate(void)
{
  ...
  if((p = (edp_pkt *)MEM_Malloc(sizeof(edp_pkt))) != NULL)
...
{
    uint8_t* pvalue_pkt = MEM_Malloc(v_len);
    Mem_Set(pvalue_pkt, '\0', v_len);
...
{
    return (uint8_t*)strcat((char*)pvalue_pkt,"{\"datastreams\":[");
}
...
uint8_t* JsonT1_PacketAddParaValue(uint8_t* pvalue_pkt, uint8_t* id, uint8_t* value, FunctionalState is_last)
{
    ...
    if (ENABLE == is_last) {
        len = sprintf((char*)tmp, "{\"id\":\"%s\",\"datapoints\":[{\"value\":%s}]}",id,value);
        if (len < 0) {
            return NULL;
        }
    }
    ...
    return (uint8_t*)strcat((char*)pvalue_pkt,(const char*)tmp);
}

uint8_t* JsonT1_PacketToEnder(uint8_t* pvalue_pkt)
{
    return (uint8_t*)strcat((char*)pvalue_pkt,"]}");
}
...
edp_pkt* JsonT1_packetDataSaveTrans(uint8_t* pvalue_pkt, const int8* destId)
{
  ...
  /* 数据类型格式1的数据长度 */
  str_len = strlen((const char*)pvalue_pkt);
  ...
  writeByte(pkt, 1);
  /* json */
  writeStr(pkt, (const int8*)pvalue_pkt);
