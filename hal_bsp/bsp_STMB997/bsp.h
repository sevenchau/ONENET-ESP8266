...
#define IO_INIT(x,y)                   BSP_PERIPH_ID_##x,x,GPIO_Pin_##y
#define IO_CTR(x,y)                    x,GPIO_Pin_##y
extern void BSP_INFLOATIO_Init(CPU_DATA  pwr_clk_id, GPIO_TypeDef* gpio, uint16_t pin);
extern void BSP_OutPPIO_Init(CPU_DATA  pwr_clk_id, GPIO_TypeDef* gpio, uint16_t pin);
extern void BSP_IO_SetBits (GPIO_TypeDef* gpio, uint16_t pin);
extern void BSP_IO_ResetBits (GPIO_TypeDef* gpio, uint16_t pin);
extern void BSP_IO_Toggle (GPIO_TypeDef* gpio, uint16_t pin);
extern uint8_t BSP_IO_ReadInPutBit(GPIO_TypeDef* gpio, uint16_t pin);

extern void USART1_Init(uint32_t BaudRate, CPU_INT08U irq_pro, void (*rx_irq_cb)(uint16_t));
extern void USART2_Init(uint32_t BaudRate, CPU_INT08U irq_pro, void (*rx_irq_cb)(uint16_t));
extern void USART1_Send(uint8_t ch);
extern void USART2_Send(uint8_t ch);
extern void USART1_RxIrqCtl(FunctionalState NewState);
extern void USART2_RxIrqCtl(FunctionalState NewState);
